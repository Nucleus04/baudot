...
const LOW_FREQUENCY_REFERENCE = 1600;
...
class Baudot extends Transform {
  ...
  detect(chunk) {
    ...
    if (frequency === 0) {
      ...
      if (
        ...
        (frequency <= LOW_FREQUENCY_REFERENCE && frequency >= 1300)
      ) {
  ...
  setCurrentFrequencyLevel(frequency) {
    ...
    if (frequency <= LOW_FREQUENCY_REFERENCE && frequency >= 1300) {
  ...
  checkIfBaudotOrNot(baudotCode) {
    if (baudotCode === "11111" || baudotCode === "11011") {
      //   console.log("baudot");
    } else {
      //   console.log("not baudot");
      this.emit("error", new Error("Stream is not Baudot"));
    }
  }
  findLetter(baudotCode) {
    if (this.isFirstConvertion) {
      this.checkIfBaudotOrNot(baudotCode);
      this.isFirstConvertion = false;
    }
